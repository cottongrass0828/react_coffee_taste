//className 的部分要考慮
export default function Input({ register, errors, id, labelText, type, rules, placeholder, labelClass, inputClass, value }) {
    ...
    return (
        <div className="w-full">
            {labelText !== '' && <label htmlFor={id} className={labelClass}>
                {labelText}
            </label>}
            ...
            <input
                ...
                value={value}
                className={`${errors[id] && 'border-red-500 ring-red-500'} ${inputClass}`}
            />
